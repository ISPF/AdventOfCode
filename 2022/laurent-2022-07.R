...
input <- read_lines(aoc_get_inputfile(7, 2022, "laurent"))
dt <- data.table(input=input)
dt[, isCmd:=ifelse(input %like% "\\$", TRUE, FALSE)]
dt[input %like% "\\$ ls", cmd:="ls"]
dt[input %like% "\\$ cd", cmd:="cd"]
dt[input %like% "dir ", cmd:="infodir"]
dt[input %like% "\\$ cd", folder:=tstrsplit(input, " ")[3]]
dt[is.na(cmd), c("size", "file"):=tstrsplit(input, " ")]
dt[is.na(cmd), cmd:="infofile"]
dt[, size:=as.integer(size)]
dt[,folder:=na.locf(folder)]
...
while(i<length(input)){
  cmd <- dt[i, cmd]
  folder <- dt[i, folder]
  if(cmd=="cd"){ #If cd change pwd and increment i by 1
    #pwd <- paste(pwd,gsub("\\$ cd ","",input[i]),sep="/")
    pwd <- paste(pwd,folder,sep="/")
    if(folder=="..") pwd <- gsub("/[^/]+/\\.\\.$","",pwd)
    #if(grepl("\\.\\.$",pwd)) pwd <- gsub("/[^/]+/\\.\\.$","",pwd)
    i <- i+1
  }
  if(cmd=="ls"){ #if ls grab content and increment by length of content
    j <- grep("^\\$",input[(i+1):length(input)])[1] #check where next command is
    #j <- dt[seq(i+1, .N)][(isCmd)][1, input]
