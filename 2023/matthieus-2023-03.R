...
dt[, input := str_replace_all(input, "\\.", " ")]
dt[, input := str_replace_all(input, "[[:punct:]]", "@")]
dt[, input := str_replace_all(input, "\\+", "@")]
dt[, input := str_replace_all(input, "\\-", "@")]
dt[, input := str_replace_all(input, "\\$", "@")]
dt[, input := str_replace_all(input, "\\=", "@")]

dt[, listOfNumbers := unique(regmatches(input, gregexpr("[[:digit:]]+", input)))]
dt[, listOfPunctuation := regmatches(input, gregexpr("[[:punct:]]+", input))]
dt$lineNumber <- 1:nrow(dt)

list_of_numbers <- dt[, .(number = unlist(listOfNumbers)), by = c("input", "lineNumber")]
list_of_numbers[, positionOfNumbers := str_locate_all(input, pattern = number)]



line = "665@   @            665          @599     @ 983      794@  14"
list_of_numbers <- unique(regmatches(line, gregexpr("[[:digit:]]+", line)))
str_locate_all(line, c(list_of_numbers))

for (j in line) {if(is.integer(j)){print(j)} else print("error")}
