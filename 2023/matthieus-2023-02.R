...
dt[, gameNumber := tstrsplit(input, ":")[1]]
dt[, gameNumber := str_replace(gameNumber, "Game ", "")]
dt[, gameList := tstrsplit(input, ":")[2]]

gameList <- dt[, .(elements = unlist(tstrsplit(gameList, ";", type.convert = TRUE))), by = "gameNumber"]
gameList[, gameNumber := as.integer(gameNumber)]

gameList[, elem1 := tstrsplit(elements, ", ")[1]]
gameList[, elem2 := tstrsplit(elements, ", ")[2]]
gameList[, elem3 := tstrsplit(elements, ", ")[3]]

gameList[is.na(gameList)] <- "0"

gameList[, `:=`(greenCount = (ifelse(grepl("green", elem1), 1, 0) * as.numeric(gsub("\\D", "", elem1)) +
                            ifelse(grepl("green", elem2), 1, 0) * as.numeric(gsub("\\D", "", elem2)) +
                            ifelse(grepl("green", elem3), 1, 0) * as.numeric(gsub("\\D", "", elem3))),
             
             redCount = (ifelse(grepl("red", elem1), 1, 0) * as.numeric(gsub("\\D", "", elem1)) +
                             ifelse(grepl("red", elem2), 1, 0) * as.numeric(gsub("\\D", "", elem2)) +
                             ifelse(grepl("red", elem3), 1, 0) * as.numeric(gsub("\\D", "", elem3))),
             
             blueCount = (ifelse(grepl("blue", elem1), 1, 0) * as.numeric(gsub("\\D", "", elem1)) +
                             ifelse(grepl("blue", elem2), 1, 0) * as.numeric(gsub("\\D", "", elem2)) +
                             ifelse(grepl("blue", elem3), 1, 0) * as.numeric(gsub("\\D", "", elem3)))
             )]

gameList[, isPossible := (redCount <= 12) & (greenCount <= 13) & (blueCount <= 14)]

compte_possible_par_game = gameList[, .(comptePossible = sum(isPossible),
                                        compte = .N), by = c("gameNumber")]
compte_possible_par_game[comptePossible  == compte, sum(gameNumber)]

min_cubes = gameList[, .(greenNeeded = max(greenCount),
                         redNeeded = max(redCount),
                         blueNeeded = max(blueCount)),
                     by = c("gameNumber")]
min_cubes[, product := greenNeeded * redNeeded * blueNeeded]
min_cubes[, sum(product)]
